...
parser.add_argument('--project_name',
                    '-n',
                    help='Porject name, name of project used by comet logger.',
                    type=str,
...
# Filter schedule
schedule = schedule.loc[schedule['job'] == job_no].drop(['job', 'estimated_time'], axis = 1)
if schedule.shape[0] == 0:
    raise Exception(f'No job marked with the following id : {args.job}.')
print(f"Running the following experiment configs {schedule}")
# Launch experiments

...
def fit_test(exp_params, data_path, k, write_path, others=None, custom_tag='',project_name = "unnamed_project"):
    ...
    # Comet experiment
    exp = Experiment(parse_args=False,project_name=project_name)
    exp.disable_mp()
    custom_tag += '_test'
    ...
    exp.set_name(f"{dataset_name}_{model_name}")
    ...
    if dim is None or dim == 2:  # Default is 2D
        m.plot(data_train, data_test, title=f'{dataset_name}')
    if dataset_name in ['Faces', 'RotatedDigits','RotatedFashion', 'UMIST', 'Tracking', 'COIL100', 'Teapot']:
        m.view_img_rec(data_train, choice='random', title=f'{dataset_name}_train_rec')
        m.view_img_rec(data_test, choice='best', title=f'{dataset_name}_test_rec_best')
        m.view_img_rec(data_test, choice='worst', title=f'{dataset_name}_test_rec_worst')
    elif dataset_name in ['ToroidalHelices', 'Mammoth'] or 'SwissRoll' in dataset_name or 'Clusters' in dataset_name:
        m.view_surface_rec(data_train, title=f'{dataset_name}_train_rec', dataset_name=dataset_name)
        m.view_surface_rec(data_test, title=f'{dataset_name}_test_rec', dataset_name=dataset_name)

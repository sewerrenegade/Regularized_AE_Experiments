...
class UMIST(BaseDataset):
    ...
    def __init__(self, split='none', split_ratio=FIT_DEFAULT, random_state=SEED, data_path=DEFAULT_PATH):
        ...
        Args:
            ...
            data_path(str, optional): Data directory.
        """
        self.url = 'https://cs.nyu.edu/home/people/in_memoriam/roweis/data/umist_cropped.mat'
...
class Rotated(BaseDataset):
    ...
    def __init__(self, fetcher, split='none', split_ratio=FIT_DEFAULT,
        ...
        to_torch_transform = transforms.Compose([
            transforms.ToTensor(),
        ])

        train = fetcher(root=data_path, train=True, download=True,
                        transform=to_torch_transform)

        X = train.data.detach().numpy()
...
class RotatedFashion(Rotated):
    ...
    def __init__(self, split='none', split_ratio=FIT_DEFAULT, random_state=SEED,
        ...
        # Pick digits that aren't rotation invariant (e.g. 1 with a 180 degree rotation is too close to
        # the base image. 6 and 9 rotated at 180 degrees can also be confusin.)
        super().__init__(torch_datasets.FashionMNIST, split, split_ratio, random_state,
                         n_images, n_rotations, data_path=data_path, classes=[2, 3, 5, 7])

        if ALLOW_CONV:
            self.data = self.data.view(-1, 1, 28, 28)
class RotatedCIFAR(Rotated):
    """3 rotated CIfar images with 360 rotations per image, for a total of
    1080 samples."""
    ...
    def __init__(self, split='none', split_ratio=FIT_DEFAULT, random_state=SEED,
        ...
        Args:
            ...
            data_path(str, optional): Data directory.
        """
        super().__init__(torch_datasets.CIFAR10, split, split_ratio, random_state,
                         n_images, n_rotations, data_path=data_path, classes=[2, 3, 5, 7])
...
class COIL100(BaseDataset):
    ...
    def __init__(self, split='none', split_ratio=FIT_DEFAULT,
        ...
        Args:
            ...
            data_path(str, optional): Data directory.
        """
        self.url = 'https://www.kaggle.com/api/v1/datasets/download/jessicali9530/coil100'#'http://www.cs.columbia.edu/CAVE/databases/SLAM_coil-20_coil-100/coil-100/coil-100.zip'
    ...
    def _download(self):
        ...
        for i in range(1, 101):
            for j in range(0, 360, 5):
                img = Image.open(os.path.join(self.root, 'coil-100', f'obj{i}__{j}.png')) #'coil-100', 'coil-100',
                img.load()
